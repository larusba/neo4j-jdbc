...
import org.neo4j.jdbc.impermanent.ImpermanentNeo4jPreparedStatement;
 ...
 * @since 3.2.0
 */
public class ImpermanentNeo4jConnectionTest {

	@Rule
	public ExpectedException expectedEx = ExpectedException.none();

	private ImpermanentNeo4jConnection openConnection;
	private ImpermanentNeo4jConnection closedConnection;
	private ImpermanentNeo4jConnection slowOpenConnection;
	private ImpermanentNeo4jConnection exceptionOpenConnection;

	@Before public void tearUp() throws SQLException {
		openConnection = new ImpermanentNeo4jConnection(mockAvailableGraphDB());
		closedConnection = mockConnectionClosed();
		slowOpenConnection = new ImpermanentNeo4jConnection(mockSlowGraphDB());
		exceptionOpenConnection = new ImpermanentNeo4jConnection(mockExceptionExecutionGraphDB());
	}
	...
	@Test public void closeShouldCloseConnection() throws SQLException {
		...
		Connection connection = new ImpermanentNeo4jConnection(db);
	...
	@Test public void prepareStatementNoParamsShouldReturnNewStatement() throws SQLException {
		...
		assertTrue(statement instanceof ImpermanentNeo4jPreparedStatement);
		assertEquals(openConnection.getHoldability(), statement.getResultSetHoldability());
		assertEquals(ResultSet.CONCUR_READ_ONLY, statement.getResultSetConcurrency());
		assertEquals(ResultSet.TYPE_FORWARD_ONLY, statement.getResultSetType());
	}
	...
	@Test public void setAutoCommitShouldThrowExceptionWhenDatabaseAccessErrorOccurred() throws SQLException {
		...
		new ImpermanentNeo4jConnection(mockExceptionGraphDB()).setAutoCommit(false);
	}
