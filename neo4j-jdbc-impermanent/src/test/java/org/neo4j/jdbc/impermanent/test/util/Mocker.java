 ...
 * Created on 11/12/17
 */
package org.neo4j.jdbc.impermanent.test.util;
...
public class Mocker {
	/**
	 * Create a mocked impermanent graph database that simulate an open connection.
	 *
	 * @return A mocked impermanent database
	 */
	...
	/**
	 * Create a mocked impermanent graph database that simulate a closed connection.
	 *
	 * @return A mocked impermanent database
	 */
	...
	/**
	 * Create a mocked impermanent graph database that simulate a errors when calling execute method.
	 *
	 * @return A mocked impermanent database
	 */
	...
	/**
	 * Create a mocked impermanent graph database that simulate a slow connection.
	 *
	 * @return A mocked impermanent database
	 */
	...
	public static ImpermanentNeo4jConnection mockConnectionOpen() throws SQLException {
		ImpermanentNeo4jConnection mockConnection = mock(ImpermanentNeo4jConnection.class);
		when(mockConnection.isClosed()).thenReturn(false);
		return mockConnection;
	}

	public static ImpermanentNeo4jConnection mockConnectionClosed() throws SQLException {
		ImpermanentNeo4jConnection closedConnection = new ImpermanentNeo4jConnection(mockUnavailableGraphDB());
		ImpermanentNeo4jConnection mockConnection = spy(closedConnection);
		doReturn(true).when(mockConnection).isClosed();
		return mockConnection;
	}

	public static ImpermanentNeo4jConnection mockConnectionOpenWithTransactionThatReturns(Result cur) throws SQLException {
	...
	public static ImpermanentNeo4jConnection mockConnectionWithExecuteThatReturns(ResultWrapper rw) throws SQLException {
		ImpermanentNeo4jConnection mockConnection = mockConnectionOpen();
