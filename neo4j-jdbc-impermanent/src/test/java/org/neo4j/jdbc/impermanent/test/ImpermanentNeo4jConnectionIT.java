...
public class ImpermanentNeo4jConnectionIT {
	@Rule
	public ExpectedException expectedEx = ExpectedException.none();

	private String NEO4J_JDBC_IMPERMANENT_URL;

	@Before public void setup() throws ClassNotFoundException {
		NEO4J_JDBC_IMPERMANENT_URL = "jdbc:neo4j:mem";
	}

	@Test public void differentConnectionShouldBeOnSameDatabase() throws SQLException {
		ImpermanentNeo4jConnection conn1 = (ImpermanentNeo4jConnection) DriverManager.getConnection(NEO4J_JDBC_IMPERMANENT_URL);
		ImpermanentNeo4jConnection conn2 = (ImpermanentNeo4jConnection) DriverManager.getConnection(NEO4J_JDBC_IMPERMANENT_URL);
	...
	@Test public void commitShouldWorkFineMultiThread() throws SQLException, InterruptedException {
		...
		Thread writerThread = new Thread() {
			public void run() {
				try {
					Connection writer = DriverManager.getConnection(NEO4J_JDBC_IMPERMANENT_URL);
		...
		Thread readerThread = new Thread() {
			public void run() {
				try {
					Connection reader = DriverManager.getConnection(NEO4J_JDBC_IMPERMANENT_URL);
